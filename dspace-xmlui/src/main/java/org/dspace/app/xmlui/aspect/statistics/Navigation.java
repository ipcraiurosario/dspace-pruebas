 ...
 *
 * @author kevinvandevelde (kevin at atmire.com) Date: 2-nov-2009 Time: 14:24:21
 */
public class Navigation extends AbstractDSpaceTransformer implements CacheableProcessingComponent {
	...
	public void addOptions(Options options)
		 ...
		 * aspects, even if they are never used
		 */
		...
		if (dso != null && dso.getHandle() != null) {
			...
			if (displayUsageStats) {
				statistics.addItemXref(contextPath + "/handle/" + dso.getHandle() + "/statistics",
						T_statistics_usage_view);
			}
			// Items cannot have search statistics
			if (displaySearchStats && dso.getType() != Constants.ITEM) {
				statistics.addItemXref(contextPath + "/handle/" + dso.getHandle() + "/search-statistics",
						T_statistics_search_view);
			}
			// Items cannot have workflow statistics
			if (displayWorkflowStats && dso.getType() != Constants.ITEM) {
				statistics.addItemXref(contextPath + "/handle/" + dso.getHandle() + "/workflow-statistics",
						T_statistics_workflow_view);
			}

		} else {
			// This Navigation is only called either on a DSO related page, or
			// the homepage
	...
	protected boolean displayStatsType(Context context, String type, DSpaceObject dso) throws SQLException {
		...
		return !cs.getPropertyAsType("usage-statistics.authorization.admin." + type, Boolean.TRUE)
    ...
    AuthorizeException
    {
	 // Arvo: Añadimos en current-locale
	Request request = ObjectModelHelper.getRequest(objectModel);
        String[] localeSplit=((String) request.getSession().getAttribute("locale-attribute")).split("_");
        Locale locale=null;
        if(localeSplit.length==1){
            locale = new Locale(localeSplit[0]);
        }else if(localeSplit.length>1){
            locale = new Locale(localeSplit[0],localeSplit[1]);
        }
        
        if(locale!=null){
            userMeta.addMetadata("locale","ISO639-1").addContent(locale.getLanguage());
            String iso3letras=null;
            try {
		iso3letras=locale.getISO3Language();
	    } catch (MissingResourceException e) {
		
	    }
            if(iso3letras!=null){
        	userMeta.addMetadata("locale","ISO639-2").addContent(iso3letras);
            }
        }        
    }
