...
importClass(Packages.es.arvo.dspace.orciduploader.UploadToOrcid);
...
function doUpdateProfile()
{
    ...
    do {
        ...
        else if(cocoon.request.get("uploadorcid")){
        	do{
            	AuthenticationUtil.log("INICIO");
            	
            	var handles = cocoon.request.getParameterValues("orcid-author-item-checkbox");
            	AuthenticationUtil.log(handles);
            	
            	UploadToOrcid.executeFromJS(handles);
            	
            	cocoon.redirectTo("profile/orciduploaded",true);
            } while(errors.length > 0 || !cocoon.request.get("uploadorcid"))
          
            	AuthenticationUtil.log("FIN");
        }
