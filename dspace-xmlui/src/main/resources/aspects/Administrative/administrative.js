...
importClass(Packages.org.dspace.app.xmlui.aspect.eperson.sipimport.ControlledVocabularyGestionUtils);
...
/**
 * Return whether the currently authenticated eperson is a Proveedor de contenidos.
 */
function isControlledVocabularyImporter() {
	return ControlledVocabularyGestionUtils.isControlledVocabularyImportAllowed(getDSContext());
}
function isControlledVocabularyExporter() {
	return ControlledVocabularyGestionUtils.isControlledVocabularyImportAllowed(getDSContext());
}

/**
 * Comprueba si tiene permisos para subir sips
 */
function assertControlledVocabularyImportAllowed() {

	if ( ! isControlledVocabularyImporter()) {
 ...
 * Comprueba si tiene permisos para subir sips
 */
function assertControlledVocabularyExportAllowed() {

	if ( ! isControlledVocabularyExporter()) {
		sendPage("admin/not-authorized");
		cocoon.exit();
	}
}
function doControlledVocabularyExport()
{
    var result;
        
    assertControlledVocabularyExportAllowed();

        sendPageAndWait("controlledvocabularyexport/main",{},result);
        result = null;

        result = doControlledVocabularyExportDownload(cocoon.request.get("select"));
        return null;
}
function doControlledVocabularyExportDownload(tablename){
	cocoon.redirectTo(cocoon.request.getContextPath()+"/tableexport/"+tablename,true);
	getDSContext().complete();
	cocoon.exit();
}

function doControlledVocabularyImport()
{
    var result;
        
    assertControlledVocabularyImportAllowed();
    do
    {
        sendPageAndWait("controlledvocabularyimport/main",{},result);
        result = null;

        if (cocoon.request.get("submit_upload") )
        {
            result = doControlledVocabularyImportUpload();

        }else  if (cocoon.request.get("submit_existente")){
        	 result = doControlledVocabularyImportExistente(cocoon.request.get("select"));
        }

    } while (true);
}

function doControlledVocabularyImportUpload()
{
    var result = ControlledVocabularyGestionUtils.preProcessUploadFile(getDSContext(),cocoon.request);

    assertControlledVocabularyImportAllowed();
    do
    {
        sendPageAndWait("controlledvocabularyimport/upload",{},result);
        result = null;
        ...
        else if (cocoon.request.get("submit_confirm"))
        {
            ...
            result = doControlledVocabularyImportConfirm();
            return result;
        }
    } while (true);
}

function doControlledVocabularyImportConfirm()
{
	assertControlledVocabularyImportAllowed();
    var result = ControlledVocabularyGestionUtils.processUploadFile(getDSContext(),cocoon.request);
   	sendPageAndWait("controlledvocabularyimport/confirm",{},result);
    return null;
}
function startControlledVocabularyImport()
{
	assertControlledVocabularyImportAllowed();

	doControlledVocabularyImport();

	cocoon.redirectTo(cocoon.request.getContextPath());
        getDSContext().complete();
	cocoon.exit();
}
function startControlledVocabularyExport()
{
	assertControlledVocabularyExportAllowed();

	doControlledVocabularyExport();
