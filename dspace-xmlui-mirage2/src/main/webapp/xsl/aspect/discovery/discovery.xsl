...
-->
    ...
    <xsl:template name="itemSummaryList">
        ...
        <div class="row ds-artifact-item ">
                            ...
                            <xsl:when test="dri:list[@n=(concat($handle, ':dc.contributor.advisor'))]">
                                <xsl:for-each select="dri:list[@n=(concat($handle, ':dc.contributor.advisor'))]/dri:item">
                                    ...
                                    <span>
                                        ...
                                        <xsl:if test="$metsDoc/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim/dim:field[@element='contributor' and @qualifier='advisor' and . = $author]/@authority">
			...
			<!--  HBC: 20190211 Se comenta el full text porque en discovery esta pintando este valor en vez de description-->
					...
					<xsl:when test="dri:list[@n=(concat($handle, ':dc.description'))]/dri:item">
                        <div class="abstract">
                                <xsl:value-of select="util:shortenString(dri:list[@n=(concat($handle, ':dc.description'))]/dri:item[1], 220, 10)"/>
                        </div>
                    </xsl:when>
                    </xsl:choose>
					<span class="item-properties">
				<xsl:choose>
					<xsl:when test="contains(dri:list[@n=(concat($handle, ':dc.rights.accesRights'))]/dri:item, 'openAccess')or contains(dri:list[@n=(concat($handle, ':dc.identifier'))]/dri:item,'revistas.urosario.edu.co')">
                        <img src="{concat($theme-path, '/images/ur/10px-openaccess.png')}" >
							<xsl:attribute name="title">
								<xsl:value-of select="dri:list[@n=(concat($handle, ':dc.rights.acceso'))]/dri:item"/>
							</xsl:attribute>
						</img>
					</xsl:when>
					<xsl:otherwise>
				...
				<i18n:text>&#160;</i18n:text>
				<xsl:value-of select="dri:list[@n=(concat($handle, ':dc.type.spa'))]/dri:item"/>
				<i18n:text>.&#160;</i18n:text>
