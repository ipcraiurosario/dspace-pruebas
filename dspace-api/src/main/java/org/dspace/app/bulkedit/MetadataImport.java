...
//                        c.commit();
                         ...
                         String md, BulkEditChange changes, DSpaceCSVLine line) throws SQLException, AuthorizeException
    {
        ...
        for (int v = 0; v < fromCSV.length; v++) {
            ...
            if ((value != null) && (!"".equals(value)) && (!containsIgnoringAuthority(value, dcvalues))) {
        ...
        for (String value : dcvalues)
        {
            ...
            if ((value != null) && (!"".equals(value)) && (!contains(value, fromCSV)) && (fromAuthority==null || !md.startsWith("ORCID:")))
	...
	protected BulkEditMetadataValue getBulkEditValueFromCSV(String language, String schema, String element,
			String qualifier, String value, AuthorityValue fromAuthority) {
		// Look to see if it should be removed
		BulkEditMetadataValue dcv = new BulkEditMetadataValue();
		dcv.setSchema(schema);
		dcv.setElement(element);
		...
		if (fromAuthority != null) {
			...
			if (valueSplit.length > 2) {
				try {
					confidence = Integer.parseInt(valueSplit[2]);
				} catch (NumberFormatException e) {
					/* Nada */}
			}

			if (id == null || confidence == null) {
				MetadataAuthorityService mam = ContentAuthorityServiceFactory.getInstance()
						.getMetadataAuthorityService();
				String fieldKey = mam.makeFieldKey(schema, element, qualifier);
				String text;
				if (mam.isAuthorityControlled(fieldKey)) {
					ChoiceAuthorityService choiceAuthorityService = ContentAuthorityServiceFactory.getInstance()
							.getChoiceAuthorityService();
					Choices choice = choiceAuthorityService.getBestMatch(fieldKey, value, null, null);
					id = choice.values.length > 0 ? choice.values[0].authority : null;
					confidence = choice.confidence;
					text = choice.values.length > 0 ? choice.values[0].value : value;
				} else {
					text = value;
				}
				if (StringUtils.isEmpty(id)) {
					...
					dcv.setConfidence(Choices.CF_UNCERTAIN);
				} else {
					dcv.setAuthority(id);
		...
		} else if (value == null || !value.contains(csv.authoritySeparator)) {
			simplyCopyValue(value, dcv);
		} else {
			String[] parts = value.split(csv.escapedAuthoritySeparator);
			dcv.setValue(parts[0]);
			dcv.setAuthority(parts[1]);
			dcv.setConfidence((parts.length > 2 ? Integer.valueOf(parts[2]) : Choices.CF_UNCERTAIN));
		}
	...
	/**
	 * Method to find if a String occurs in an array of Strings (ignoring ::)
	 *
	 * @param needle
	 ...
	 * @return Whether or not it is contained
	 */
	private boolean containsIgnoringAuthority(String needle, String[] haystack) {
		// Look for the needle in the haystack
		for (String examine : haystack) {
			if (examine.contains("::")) {
				...
				if (examine.split("::")[0].equals(needle.split("::")[0])) {
					return true;
				}
			}
		}
	...
	private static boolean isAuthorityControlledField(String md) {
		String mdf = md;
		if (mdf.contains(":")) {
			mdf = StringUtils.substringAfter(md, ":");
		}
		if (mdf.contains("[")) {
