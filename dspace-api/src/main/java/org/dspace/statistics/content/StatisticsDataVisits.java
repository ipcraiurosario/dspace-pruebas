...
//                System.out.println("BOTH");
            ...
            String filterQuery) throws SolrServerException
    {
        String facetType = dataset.getFacetField() == null ? "id" : dataset
                .getFacetField();
     	// ARVO: Añadido el incluir totales del primer generador de resultados
        boolean incluirTotal=getDatasetGenerators().size()>0?getDatasetGenerators().get(0).includeTotal:false;
        return solrLoggerService.queryFacetField(query, filterQuery, facetType,
        		dataset.getMax(), incluirTotal, null);
    }
                ...
                if(owningDso != null && currentDso != null){
                    ...
                    if (dsoType == -1) {
                        if(currentDso instanceof DSpaceObjectLegacySupport){
                            query += " (id:" + currentDso.getID() + " OR id:" + ((DSpaceObjectLegacySupport) currentDso).getLegacyId() + ")";
                        }else{
                            query += "id:" + currentDso.getID();
                        }
                        query += " OR ";
                    }
