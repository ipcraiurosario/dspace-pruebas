...
public class BitstreamStorageServiceImpl implements BitstreamStorageService, InitializingBean
{
	...
	@Override
	public File getFile(Bitstream bitstream) {
		BitStoreService bitStoreService = stores.get(bitstream.getStoreNumber());
		if(bitStoreService instanceof DSBitStoreService){
			DSBitStoreService dsBitStoreService = (DSBitStoreService) bitStoreService;
			try {
				return dsBitStoreService.getFile(bitstream);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
