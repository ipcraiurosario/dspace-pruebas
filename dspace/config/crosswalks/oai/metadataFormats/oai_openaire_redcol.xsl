...
<xsl:stylesheet
	...
	<xsl:template match="/">
		<oaire:resource xmlns:rdf="http://www.w3.org/TR/rdf-concepts/"
			...
			<oaire:resourceType resourceTypeGeneral="literature">
					...
					<xsl:call-template name="getTypeValue">
	...
	<xsl:template name="getTypeUri">
		...
		<xsl:choose>
			...
			<xsl:when test="$type='Artículo' or $type='article'">
				<xsl:text>http://purl.org/coar/resource_type/c_6501</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Trabajo de grado de pregrado' or $type='bachelorThesis'">
				<xsl:text>http://purl.org/coar/resource_type/c_7a1f</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Libro' or $type='book'">
				<xsl:text>http://purl.org/coar/resource_type/c_2f33</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Parte de libro' or $type='bookPart'">
				<xsl:text>http://purl.org/coar/resource_type/c_3248</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Objeto de conferencia' or $type='conferenceObject'">
				<xsl:text>http://purl.org/coar/resource_type/c_c94f</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Tesis doctoral' or $type='doctoralThesis'">
				<xsl:text>http://purl.org/coar/resource_type/c_db06</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Conferencia' or $type='lecture'">
				<xsl:text>http://purl.org/coar/resource_type/c_8544</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Tesis de maestría' or $type='masterThesis'">
				<xsl:text>http://purl.org/coar/resource_type/c_bdcc</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Preimpresión' or $type='preprint'">
				<xsl:text>http://purl.org/coar/resource_type/c_816b</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Reporte técnico' or $type='technical report'">
				<xsl:text>http://purl.org/coar/resource_type/c_18gh</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Reporte' or $type='report'">
				<xsl:text>http://purl.org/coar/resource_type/c_93fc</xsl:text>
			</xsl:when>
			<xsl:when test="$type='Revisión' or $type='review'">
			...
			<xsl:when test="$type='Video' or $type='video'">
			...
			<xsl:when test="$type='Documento de trabajo' or $type='workingPaper'">
	...
	<xsl:template name="getTypeValue">
		<xsl:param name="typeValue" />
		<xsl:choose>
			<xsl:when test="$typeValue='Artículo de revista'">
				<xsl:text>journal article</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Artículo' or $typeValue='article'">
				<xsl:text>journal article</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Trabajo de grado de pregrado' or $typeValue='bachelorThesis'">
				<xsl:text>bachelor thesis</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Libro' or $typeValue='book'">
				<xsl:text>book</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Parte de libro' or $typeValue='bookPart'">
				<xsl:text>book part</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Objeto de conferencia' or $typeValue='conferenceObject'">
				<xsl:text>conference object</xsl:text>
			</xsl:when> 
			<xsl:when test="$typeValue='Tesis doctoral' or $typeValue='doctoralThesis'">
				<xsl:text>doctoral thesis</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Conferencia' or $typeValue='lecture'">
				<xsl:text>lecture</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Tesis de maestría' or $typeValue='masterThesis'">
				<xsl:text>master thesis</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Preimpresión' or $typeValue='preprint'">
				<xsl:text>preprint</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Reporte técnico' or $typeValue='technical report'">
				<xsl:text>technical report</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Reporte' or $typeValue='report'">
				<xsl:text>report</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Revisión' or $typeValue='review'">
				<xsl:text>review</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='annotation'">
				<xsl:text>annotation</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='contributionToPeriodical'">
				<xsl:text>contribution to journal</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='patent'">
				<xsl:text>patent</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Video' or $typeValue='video'">
				<xsl:text>video</xsl:text>
			</xsl:when>
			<xsl:when test="$typeValue='Documento de trabajo' or $typeValue='workingPaper'">
	...
	<xsl:template name="resolveResourceTypeGeneral">
        ...
        <xsl:choose>
            <xsl:when test="$field='Artículo' or $field='article'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
            <xsl:when test="$field='Artículo de revista'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
			<xsl:when test="$field='Libro' or $field='book'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
			<xsl:when test="$field='Parte de libro' or $field='bookPart'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
            <xsl:when test="$field = 'book review'">
                <xsl:text>literature</xsl:text>
            </xsl:when>
            <xsl:when test="$field = 'dataset'">
                <xsl:text>dataset</xsl:text>
            </xsl:when>
            <xsl:when test="$field = 'software'">
