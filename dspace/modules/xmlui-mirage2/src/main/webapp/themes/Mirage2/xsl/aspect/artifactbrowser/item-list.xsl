...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1" xmlns:dri="http://di.tamu.edu/DRI/1.0/" xmlns:mets="http://www.loc.gov/METS/"
    xmlns:dim="http://www.dspace.org/xmlns/dspace/dim" xmlns:xlink="http://www.w3.org/TR/xlink/" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:encoder="xalan://java.net.URLEncoder" xmlns:util="org.dspace.app.xmlui.utils.XSLUtils" xmlns:confman="org.dspace.core.ConfigurationManager"
    ...
    <xsl:template name="itemSummaryList-DIM">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:apply-templates select="./mets:dmdSec/mets:mdWrap[@OTHERMDTYPE='DIM']/mets:xmlData/dim:dim" mode="itemSummaryList-DIM-metadata">
                    <xsl:with-param name="href" select="$href" />
                </xsl:apply-templates>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="dim:dim" mode="itemSummaryList-DIM-metadata">
        ...
        <div class="artifact-description">
            ...
            <xsl:if test="dim:field[@element = 'description' and not(@qualifier)]">
                <xsl:variable name="abstract" select="dim:field[@element = 'description' and not(@qualifier)]/node()" />
                <div class="artifact-abstract">
                    <xsl:value-of select="util:shortenString($abstract, 220, 10)" />
                </div>
            </xsl:if>

            <xsl:if test="not(dim:field[@element = 'description' and not(@qualifier)]) and dim:field[@element = 'description' and @qualifier='abstract']">
            ...
            <span class="item-properties">
                <xsl:choose>
                    <xsl:when test="contains(dim:field[@element='rights' and @qualifier='accesRights'][1]/node(), 'info:eu-repo/semantics/openAccess') or contains(dim:field[@element='identifier'][not(@qualifier)], 'revistas.urosario.edu.co')">
                        <img src="{concat($theme-path, '/images/ur/10px-openaccess.png')}">
                        <xsl:attribute name="title">
                            <xsl:value-of select="dim:field[@element='rights' and @qualifier='acceso'][1]/node()" />
                        </xsl:attribute>
                        </img>
                    </xsl:when>
                    <xsl:otherwise>
                        <img src="{concat($theme-path, '/images/ur/10px-closedaccess.png')}">
                        <xsl:attribute name="title">
                            <xsl:value-of select="dim:field[@element='rights' and @qualifier='acceso'][1]/node()" />
                        </xsl:attribute>
                        </img>
                ...
                <i18n:text>&#160;</i18n:text>
                <xsl:value-of select="dim:field[@element='type' and @qualifier='spa'][1]/node()" />
                <i18n:text>.&#160;</i18n:text>
                <xsl:value-of select="substring(dim:field[@element='date' and @qualifier='created']/node(),1,4)" />
                <!--<xsl:call-template name="plumx-item-list">
                     					<xsl:with-param name="identifier_uri" select="dim:field[@element='identifier' and @qualifier='uri']"/>
				</xsl:call-template>-->
            </span>
        </div>
    </xsl:template>
    <!--	<xsl:template name="plumx-item-list">
	...
	<div class="plumx-altmetrics">
		<a>
			...
			<xsl:attribute name="data-popup">right</xsl:attribute>
			<xsl:attribute name="data-hide-when-empty">true</xsl:attribute>
			<xsl:attribute name="data-site">urosario</xsl:attribute>
			<xsl:attribute name="data-size">small</xsl:attribute>
			<xsl:attribute name="data-badge">true</xsl:attribute>
			<xsl:attribute name="data-lang">es</xsl:attribute>
			<xsl:attribute name="data-pass-hidden-categories">true</xsl:attribute>
	...
	<!--<xsl:param name="mimetype" />-->
        <xsl:variable name="mimetype" select="dim:field[@element='format' and @qualifier='mimetype'][1]/node()" />
