...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="addLookupButton">
      ...
      <!-- optional param for indicate the limit of choices to show when windows popup-->
      <xsl:param name="choicesLimit" />
      <!-- optional param for indicate selected values from popup are read-only -->
      <xsl:param name="isReadOnly" />
      
	  <xsl:variable name="limitOfChoices">    
	      <xsl:choose>
	      	<xsl:when test="$choicesLimit != ''">
	      		<xsl:value-of select="$choicesLimit" />
	      	</xsl:when>
	      	<xsl:otherwise>
	      		<!-- by default, 50 is the limit of choices... -->
	  ...
	  <xsl:variable name="readOnly">    
	      <xsl:choose>
	      	...
	      	<xsl:otherwise>
	      		<!-- by default, this field can be modified (not "read-only") -->
      ...
      <input type="button" name="{concat('lookup_',@n)}" class="pull-right ds-button-field btn btn-default ds-add-button" >
        ...
        <xsl:attribute name="onClick">
          ...
          <xsl:text>, </xsl:text>
          <!-- choicesLimit -->
          <xsl:value-of select="$limitOfChoices"/>
          <xsl:text>, </xsl:text>
          <!-- isReadOnly -->
    ...
    <xsl:template name="authorityInputFields">
      ...
      <input class="ds-authority-confidence-input" type="hidden">
        ...
        <xsl:attribute name="value">
			<xsl:if test="$confValue='' or $confValue='blank'">
				 <xsl:text>ACCEPTED</xsl:text>
			</xsl:if>
			<xsl:if test="$confValue!='blank'">
